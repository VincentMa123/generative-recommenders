...
class VQEmbedding(torch.nn.Embedding):
    ...
    def _create_codebook(self, data):
        """
        Create codebook using specified clustering method
        """
        n_samples = data.size(0)
        effective_clusters = min(self.num_clusters, n_samples)

        if self.kmeans_method == 'kmeans':
            _codebook, _ = kmeans(data, effective_clusters, self.kmeans_iters)
            ...
            # Default to regular kmeans if method not recognized
            _codebook, _ = kmeans(data, effective_clusters, self.kmeans_iters)

        # If we have fewer clusters than requested, pad with duplicates
        if effective_clusters < self.num_clusters:
            # Repeat the last center or use random selection from existing
            if effective_clusters > 0:
                # Option 1: Repeat the last center
                pad_centers = _codebook[-1:].expand(self.num_clusters - effective_clusters, -1)
                _codebook = torch.cat([_codebook, pad_centers], dim=0)
            else:
                # Fallback: create random codebook if no data
                _codebook = torch.randn(self.num_clusters, self.codebook_emb_dim)

        # Ensure correct device and dtype
        _codebook = _codebook.to(device=self.device, dtype=data.dtype)

        # Debug: Print shapes before assertion
        #print(f"Expected codebook shape: ({self.num_clusters}, {self.codebook_emb_dim})")
        #print(f"Actual codebook shape: {_codebook.shape}")
        #print(f"Data shape: {data.shape}")
        #print(f"Effective clusters: {effective_clusters}")

        # Verify final shape
        assert _codebook.shape == (self.num_clusters, self.codebook_emb_dim), \
            f"Codebook shape {_codebook.shape} doesn't match expected ({self.num_clusters}, {self.codebook_emb_dim})"
    ...
    def _compute_distances(self, data):
        data = data.to(self.device)
        _codebook_t = self.codebook.t().to(self.device, dtype=data.dtype)
